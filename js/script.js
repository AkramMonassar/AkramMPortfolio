...
const translations = {
    en: {
        title: "Akram Monassar | Senior Field & Technical Operations Specialist & Full-Stack Developer",
        ...
        keywords: "Akram Monassar, Field & Technical Operations Specialist, M&E Researcher, Full Stack Developer, Software Engineering, IT Infrastructure, ODK, KoBo, SPSS, Ibb Yemen"
    },
    ar: {
        ...
        keywords: "أكرم منصر, أخصائي عمليات ميدانية وتقنية, باحث رقابة وتقييم, مطور ويب متكامل, هندسة البرمجيات, البنية التحتية, ODK, KoBo, SPSS, إب اليمن"
    }
};
    ...
    function setLanguage(lang) {
        ...
        if (metaKeywords) metaKeywords.setAttribute('content', translations[lang].keywords);
    ...
    themeToggleBtn.addEventListener('click', () => {
        const currentTheme = bodyTag.classList.contains('light-theme') ? 'dark' : 'light';
        ...
        contactForm.addEventListener('submit', (e) => {
            ...
            if (isFormValid) {
                ...
                setTimeout(() => {
                    ...
                    // Hide success prompt after 5 seconds
                    setTimeout(() => {
                        feedbackSuccess.style.display = 'none';
                    }, 5000);
                }, 1500); // Mock network lag
            } else {
                feedbackSuccess.style.display = 'none';
    ...
    if (downloadResumeBtn && resumeModal) {
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape' && resumeModal.classList.contains('active')) closeModal();
        });
...
document.querySelectorAll('.project-slider').forEach(slider => {
    ...
    if (slides.length <= 1) {
        slider.querySelectorAll('.slider-arrow, .slider-dots').forEach(el => el.style.display = 'none');
        const singleImg = slides[0].querySelector('img');
        singleImg.addEventListener('click', () => openLightbox([singleImg.getAttribute('src')], 0));
        return;
    }
    let current = 0;
    const go = (i) => {
    ...
    const restartAuto = () => { clearInterval(auto); auto = setInterval(() => go(current + 1), 4000); };
    slider.addEventListener('mouseenter', () => clearInterval(auto));
    slider.addEventListener('mouseleave', restartAuto);
    slider.querySelector('.next').addEventListener('click', (e) => { e.preventDefault(); go(current + 1); restartAuto(); });
    slider.querySelector('.prev').addEventListener('click', (e) => { e.preventDefault(); go(current - 1); restartAuto(); });
    // فتح الـ Lightbox عند الضغط على الصورة
    const imgs = Array.from(slides).map(s => s.querySelector('img').getAttribute('src'));
    slides.forEach((s, i) => {
        s.querySelector('img').addEventListener('click', () => openLightbox(imgs, i));
    });
});
